  ...
  omni_target_priority_{ArmorPriority::fifth}
{
  ...
  lightbar_assist_enabled_ = false;
  ...
  auto load_reprojection_config = [&]() {
    ...
    reprojection_config_.use_adaptive_noise =
  ...
  if (observation_mode == "reprojection") {
    ...
    if (filter_method_ == FilterMethod::EKF) {
      lightbar_assist_enabled_ = true;
      debug_info_.lightbar_assist_enabled = true;
      load_reprojection_config();
      tools::logger()->info("[Tracker] PnP lightbar assist enabled");
    } else {
      tools::logger()->warn(
        "[Tracker] pnp.enable_lightbar_assist requires EKF; keeping original PnP path");
    }
  }
...
std::vector<ReprojectionMeasurement> Tracker::match_independent_lightbars(
  ...
  const std::vector<int> & visible_ids, const std::vector<int> & occupied_ids)
{
  ...
  if (target_.name == ArmorName::base || predicted_armors.empty() || visible_ids.empty()) {
    return measurements;
  }
  ...
  DetectionResult & detections, std::chrono::steady_clock::time_point t)
{
  ...
  if (!matched_armors.empty() && target_.name != ArmorName::base) {
    const std::vector<int> occupied_ids = [&]() {
      std::vector<int> ids;
      ids.reserve(matched_armors.size());
      for (const auto & [id, armor] : matched_armors) {
        (void)armor;
        ids.push_back(id);
      }
    ...
    const auto independent_measurements = match_independent_lightbars(
      detections, predicted_armors, visible_ids, occupied_ids);
    measurements.insert(
...
bool Tracker::update_target(DetectionResult & detections, std::chrono::steady_clock::time_point t)
{
  ...
  for (auto & armor : armors) {
    ...
    occupied_ids.push_back(target_.last_id);
  }

  if (found && lightbar_assist_enabled_) {
    const auto predicted_armors = target_.armor_xyza_list();
    ...
    if (!measurements.empty() &&
        target_.update_lightbar_assist(measurements, solver_, reprojection_config_)) {
